...
@Component({
  selector: 'app-change-mou-view-detail-history-old',
  templateUrl: './change-mou-view-detail-history-old.component.html'
})
export class ChangeMouViewDetailHistoryOldComponent implements OnInit {
    ...
    await this.http.post(URLConstant.GetMouCustById, { Id: this.MouCustId }).toPromise().then(
      async (response: MouCustObj) => {
        ...
        if (this.MouType == 'FACTORING') {
          ...
          await this.http.post(URLConstant.GetForViewChangeMouCustDlrFncngDetailByChangeMouTrxId, { Id: this.ChangeMouTrxId }).toPromise().then(
