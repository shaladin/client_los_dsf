...
export class ProdOfferingApvPagingComponent implements OnInit {
  ...
  async CallBackHandler(ev) {
    ...
    if (ev.Key == "Process") {
      ...
      AdInsHelper.RedirectUrl(this.router,[NavigationConstant.PRODUCT_OFFERING_APPRV_DETAIL],{ "ProdOfferingHId": ev.RowObj.ProdOfferingHId, "TaskId" : ev.RowObj.TaskId, "ApvReqId": environment.isCore ? ev.RowObj.RequestId : ev.RowObj.ApvReqId});    
    }
