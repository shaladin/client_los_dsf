        ...
        this.http.post(URLConstantX.GetAutoDebitRegistrationByNo, { TrxNo: e.RowObj.TransactionNo }).subscribe(
          async (response: AutoDebitRegistrationObj) => {
            ...
            if (adr.BankCode == this.bankBCA) {
              const reqGsObj: object = {
                code: CommonConstantX.GS_CODE_AUTO_DEBIT_BCA_WEBVIEW_DOMAIN
              }
              const domain = await this.http.post(URLConstant.GetGeneralSettingByCode, reqGsObj)
                .toPromise()
                .then((res) => {
                  if (res['GsValue'] !== undefined && res['GsValue'] !== null && res['GsValue'] !== '') {
                    return res['GsValue']
                  }
              ...
              this.http.post(URLConstantX.GetListStgAutoDebitRegisLog, { TrxNo: e.RowObj.TransactionNo }).subscribe(
                (response1) => {
                  ...
                  if (new Date() < new Date(response1["ExpiredDt"]) && response1["RequestId"] != null) {
                    //window.open
                    this.windowSkpr = window.open("https://" + domain + "/id/skpr/registration?req-id=" + response1["RequestId"] + "&verification=" + response1["Verification"]);
                    $(document).ready(() => {
                    ...
                    this.http.post(URLConstantX.ProcessAutoDebitRegistration, obj).subscribe(
                      (response2) => {
                        if (response2["StatusCode"] != 200) {
                          ...
                          this.http.post(URLConstantX.GetListStgAutoDebitRegisLog, { TrxNo: e.RowObj.TransactionNo }).subscribe(
                            (response3) => {
                              this.windowSkpr = window.open("https://" + domain + "/id/skpr/registration?req-id=" + response3["RequestId"] + "&verification=" + response3["Verification"]);
                              $(document).ready(() => {
                                window.addEventListener("message", receiveMessage.bind(this), false);
                              });
                            }
