...
export class InvoiceDataDetailXComponent implements OnInit {
  ...
  InvoiceDataForm = this.fb.group({
    ...
    TotalInvoiceAmt: ['', Validators.required],
    RcvBy: [''],
    InvoiceAmt: [''],
    PurchaseOrderHId: ['']
  });
  ...
  async ngOnInit() {
    ...
    await this.http.post(URLConstantX.GetPurchaseOrderHByAgrmntIdAndSupplCodeX, appAssetObj).toPromise().then(
      async (response) => {
        if (response["PurchaseOrderHId"] != 0) {
          this.purchaseOrderHObj.PurchaseOrderHId = response["PurchaseOrderHId"];
          this.purchaseOrderHObj.PurchaseOrderNo = response["PurchaseOrderNo"];
          this.purchaseOrderHObj.TotalPurchaseOrderAmt = response["TotalPurchaseOrderAmt"];
        }
      });
    ...
    await this.http.post(URLConstantX.GetInvoiceXByVendorCodeAndPurchaseOrderHId, RequestInvoiceX).subscribe(
      (response) => {
        ...
        if (response['ResponseInvoiceHXObj'] != null) {
          ...
          this.InvoiceDataForm.patchValue({
            ...
            InvoiceDt: formatDate(response['ResponseInvoiceHXObj'].InvoiceDt, 'yyyy-MM-dd', 'en-US'),
  ...
  async AddEditInvoice(RequestInvoiceX: any) {
    ...
    if (!this.isDataExist) {
      this.http.post(URLConstantX.AddInvoiceX, RequestInvoiceX).subscribe(
        (response) => {
          this.toastr.successMessage(response["message"]);
          AdInsHelper.RedirectUrl(this.router, [NavigationConstant.NAP_ADM_PRCS_INVOICE_DATA], { "AgrmntId": this.AgrmntId, "AppId": this.AppId, "TaskListId": this.TaskListId, "BizTemplateCode": this.BizTemplateCode });

        });
    } else {
      this.http.post(URLConstantX.EditInvoiceX, RequestInvoiceX).subscribe(
        (response) => {
          this.toastr.successMessage(response["message"]);
          AdInsHelper.RedirectUrl(this.router, [NavigationConstant.NAP_ADM_PRCS_INVOICE_DATA], { "AgrmntId": this.AgrmntId, "AppId": this.AppId, "TaskListId": this.TaskListId, "BizTemplateCode": this.BizTemplateCode });
        });
