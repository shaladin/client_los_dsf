...
@Component({
  selector: 'app-pre-go-live-approval-paging-x',
  templateUrl: './pre-go-live-approval-paging-x.component.html'
})
export class PreGoLiveApprovalPagingXComponent implements OnInit {
  ...
  async GetCallBack(ev: any) {
    ...
    if (ev.Key == "Process") {
      ...
      AdInsHelper.RedirectUrl(this.router,[NavigationConstant.NAP_ADM_PRCS_PGL_APPRVL_DETAIL_X],{ "AgrmntId": ev.RowObj.AgrmntId, "AppId": ev.RowObj.AppId, "TrxNo": ev.RowObj.TrxNo, "TaskId" : ev.RowObj.TaskId, "InstanceId": ev.RowObj.InstanceId, "ApvReqId": environment.isCore ? ev.RowObj.RequestId : ev.RowObj.ApvReqId });
    }
