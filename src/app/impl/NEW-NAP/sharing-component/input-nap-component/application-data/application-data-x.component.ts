...
@Component({
  ...
  templateUrl: './application-data-x.component.html'
})
export class ApplicationDataXComponent implements OnInit {
  ...
  copyToMailing(addrType: string = '') {
    if (!addrType) {
      addrType = this.NapAppModelForm.controls.CopyFromMailing.value;
    }
    if (!addrType) {
      return;
    }
