...
@Component({
  selector: 'app-prod-ho-paging-x',
  templateUrl: './prod-ho-paging-x.component.html'
})
export class ProdHoPagingXComponent implements OnInit {
  InputPagingObj: UcPagingObj = new UcPagingObj();
  readonly AddLink: string = NavigationConstant.PRODUCT_HO_ADD_X;
  ...
  EditButtonClick(e) {  
    if (e.RowObj.DraftProdHId == null) {
      AdInsHelper.RedirectUrl(this.router, [NavigationConstant.PRODUCT_HO_ADD_X], { "ProdHId": e.RowObj.prodHId, "mode": "edit" });
    }
    else {
      AdInsHelper.RedirectUrl(this.router, [NavigationConstant.PRODUCT_HO_ADD_X], { "ProdHId": e.RowObj.DraftProdHId, "mode": "edit" });
    }
  }
}
