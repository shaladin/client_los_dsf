...
@Component({
  selector: 'app-app-source-add-edit-x',
  templateUrl: './app-source-add-edit-x.component.html'
})
export class AppSourceAddEditXComponent implements OnInit {
  ...
  AppSourceForm = this.fb.group({
    AppSrcCode: ['', [Validators.required, Validators.maxLength(20)]],
  ...
  readonly CancelLink: string = NavigationConstant.SETTING_APP_SOURCE_PAGING_X;
    ...
    if (this.mode == "Add") {
      ...
      this.http.post(URLConstant.AddRefAppSrc, this.rasObj).subscribe(
        response => {
          ...
          AdInsHelper.RedirectUrl(this.router, [NavigationConstant.SETTING_APP_SOURCE_PAGING_X], {});
        }
      );
    } else {
