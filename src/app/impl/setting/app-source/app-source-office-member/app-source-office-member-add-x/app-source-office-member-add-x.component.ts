...
@Component({
  selector: 'app-app-source-office-member-add-x',
  templateUrl: './app-source-office-member-add-x.component.html'
})
export class AppSourceOfficeMemberAddXComponent implements OnInit {
  ...
  SaveAppSourceOfficeMember() {
    ...
    this.http.post(URLConstant.AddRefAppSrcOfficeMbr, RequestItem).subscribe(
      (response) => {
        ...
        AdInsHelper.RedirectUrl(this.router,[NavigationConstant.SETTING_APP_SOURCE_OFFICE_MBR_PAGING_X],{ "RefAppSrcId": this.RefAppSrcId });
      });
