...
@Component({
  selector: 'app-invoice-main-info-x',
  templateUrl: './invoice-main-info-x.component.html'
})
export class InvoiceMainInfoXComponent implements OnInit {
        ...
        await this.http.post(URLConstant.GetAppById, { Id: appId }).subscribe(
          (response) => {
            if (response['BizTemplateCode'] == CommonConstant.FL4W) {
              this.viewGenericObj.viewInput = "./assets/impl/ucviewgeneric/viewInvoiceFL4WMainInfoX.json";
            }
            else if (response['BizTemplateCode'] == CommonConstant.CFNA) {
              this.viewGenericObj.viewInput = "./assets/impl/ucviewgeneric/viewInvoiceMainInfoCfnaX.json";
            }
            else {
              this.viewGenericObj.viewInput = "./assets/impl/ucviewgeneric/viewInvoiceMainInfoX.json";
            }
            this.isReady = true;
          });
      }
