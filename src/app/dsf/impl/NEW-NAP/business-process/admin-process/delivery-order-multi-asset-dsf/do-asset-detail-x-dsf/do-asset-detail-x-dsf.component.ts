...
@Component({
  selector: 'app-do-asset-detail-x-dsf',
  templateUrl: './do-asset-detail-x-dsf.component.html'
})
export class DoAssetDetailXDsfComponent implements OnInit {
  ...
  runnerActivationValue: Array<KeyValueObj> = new Array();
        ...
        await this.http.post(URLConstantDsf.GetAppAssetByAppAssetIdDsf, appAssetDsfObj).toPromise().then(
          async (response2) => {
            ...
            if(this.runnerActivationValue.length == 0)
            {
              ...
              this.DOAssetDetail.controls.selectedRunnerActivation.disable();
            }
          }
        );
        // End Self Custom Changes CR Runner KTB 398912
  ...
  async setRunnerActivation(){
    var requestObj = {
      FullAssetCode: this.DOAssetDetail.controls.FullAssetCode.value,
      AssetCondition: this.DOAssetDetail.controls.MrAssetConditionCode.value,
      CurrAppStep: CommonConstant.AppStepDO,
      AppNo: this.AppObj.AppNo
    }
    await this.http.post(URLConstantDsf.GetListRunnerActivationDsf, requestObj).toPromise().then(
        async (response) => {
          console.log(response);
          if (response != null) this.runnerActivationValue = response['ReturnObject'];
    });
  }
  // End Self Custom Changes CR Runner KTB 398912
}
