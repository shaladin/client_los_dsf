...
@Component({
  selector: 'app-purchase-order-detail-x-dsf',
  templateUrl: './purchase-order-detail-x-dsf.component.html'
})
export class PurchaseOrderDetailXDsfComponent implements OnInit {
  ...
  readonly CancelLink: string = NavigationConstant.NAP_ADM_PRCS_PO_PO_EXT_DSF;
    ...
    if (!this.isDataExist) {
      this.http.post(URLConstant.AddPurchaseOrder, POObj).subscribe(
        (response) => {
          ...
          // Self Custom Changes CR Addition Reload Button TC 459737
          AdInsHelper.RedirectUrl(this.router, [NavigationConstant.NAP_ADM_PRCS_PO_PO_EXT_DSF], { "AgrmntId": this.AgrmntId, "LobCode": this.lobCode, "AppId": this.AppId, "TaskListId": this.TaskListId, "BizTemplateCode": this.BizTemplateCode });
          // End Self Custom Changes CR Addition Reload Button TC 459737
        });
    } else {
