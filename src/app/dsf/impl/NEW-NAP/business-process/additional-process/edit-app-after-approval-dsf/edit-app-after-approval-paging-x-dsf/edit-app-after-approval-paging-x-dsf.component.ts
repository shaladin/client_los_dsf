...
@Component({
  selector: 'app-edit-app-after-approval-paging-x-dsf',
  templateUrl: './edit-app-after-approval-paging-x-dsf.component.html'
})
export class EditAppAfterApprovalPagingXDsfComponent implements OnInit, OnDestroy {
      ...
      this.http.post(URLConstantX.GetEditComReqAndApvByAgrmntId, reqObj).toPromise().then(
        (response) => {
          ...
          if (this.IsApplicationExist == false){
            ...
            AdInsHelper.RedirectUrl(this.router, [NavigationConstantDsf.NAP_ADD_PRCS_EDIT_APP_AFT_APV_DETAIL_DSF], { "AgrmntId": ev.RowObj.AgrmntId });
