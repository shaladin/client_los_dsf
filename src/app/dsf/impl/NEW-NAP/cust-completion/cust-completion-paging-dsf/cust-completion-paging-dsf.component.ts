...
@Component({
  selector: 'app-cust-completion-paging-dsf',
  templateUrl: './cust-completion-paging-dsf.component.html'
})
export class CustCompletionPagingDsfComponent implements OnInit, OnDestroy {
      ...
      this.http.post(URLConstant.CheckIsNegCustAllowedCreateAppByAppId, { Id: ev.RowObj.AppId }).subscribe(
        (res) => {
          if(res) 
            ...
            AdInsHelper.RedirectUrl(this.router, [NavigationConstantDsf.NAP_CUST_COMPL_DETAIL_DSF], { "AppId": ev.RowObj.AppId, "WfTaskListId": environment.isCore ? ev.RowObj.Id : ev.RowObj.WfTaskListId, "BizTemplateCode": ev.RowObj.BizTemplateCode });
