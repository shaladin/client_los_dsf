...
@Component({
  selector: 'app-crd-rvw-cust-info-x-dsf',
  templateUrl: './crd-rvw-cust-info-x-dsf.component.html'
})
export class CrdRvwCustInfoXDsfComponent implements OnInit {
  ...
  //#region GroupHistData
  modalGroupHistData: any;
  ClickLinkGroupHistData(GroupHistDataContent) {
    this.modalGroupHistData = this.modalService.open(GroupHistDataContent);
    this.modalGroupHistData.result.then((result) => {
      this.closeResult = `Closed with: ${result}`;
      this.modalGroupHistData.close();
    }, (reason) => {
      this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
      this.modalGroupHistData.close();
    });
