...
@Component({
  selector: 'app-agrmnt-tc-dsf',
  templateUrl: './agrmnt-tc-dsf.component.html',
  styleUrls: ['./agrmnt-tc-dsf.component.css'],
  viewProviders: [{ provide: ControlContainer, useExisting: FormGroupDirective }]
})
export class AgrmntTcDsfComponent implements OnInit {
  ...
  async refresh()
  {
    await this.http.post(URLConstantDsf.ReloadAgrmntTcDsf, { Id: this.AgrmntId }).toPromise().then(
      async (response2) => {
        var listTC = this.parentForm.get(this.identifier) as FormArray;
        while (listTC.length !== 0) {
          listTC.removeAt(0)
