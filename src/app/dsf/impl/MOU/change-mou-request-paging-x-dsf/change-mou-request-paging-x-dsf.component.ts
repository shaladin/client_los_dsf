...
export class ChangeMouRequestPagingXDsfComponent implements OnInit {
  ...
  getEvent(ev: any) {
    ...
    } else if (ev.Key == "editFactoringDsf") {
      var obj = { Id: ev.RowObj["MouCustId"] };
      this.http
        .post(URLConstant.CheckMouCustInChangeMouProcess, obj)
        .subscribe((response) => {
  ...
  ChangeMouType(mouType) {
    ...
    if (mouType == CommonConstant.FACTORING) {
      this.inputPagingObj._url = "./assets/impl/ucpaging/mou/searchChangeMouRequestFactoringXDsf.json";
      this.inputPagingObj.pagingJson = "./assets/impl/ucpaging/mou/searchChangeMouRequestFactoringXDsf.json";
