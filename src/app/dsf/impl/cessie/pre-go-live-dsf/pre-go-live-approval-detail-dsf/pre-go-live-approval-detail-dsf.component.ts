...
@Component({
  selector: 'cessie-pre-go-live-approval-detail-dsf',
  templateUrl: './pre-go-live-approval-detail-dsf.component.html'
})
export class CessiePreGoLiveApprovalDetailDsfComponent implements OnInit {
  viewObj: string;
  TrxNo: string;
  ApvReqId: number;
  taskId: number;
  ...
  HoldTask(obj: ApprovalObj) {
    this.http.post(URLConstant.ApvHoldTaskUrl, obj).subscribe(
      ...
      (error) => {
        AdInsHelper.RedirectUrl(this.router, [NavigationConstantDsf.CESSIE_PGL_APPRVL_PAGING], {});
      }
    )
  }
  ...
  onCancelClick() {
    AdInsHelper.RedirectUrl(this.router, [NavigationConstantDsf.CESSIE_PGL_APPRVL_PAGING], { "BizTemplateCode": localStorage.getItem(CommonConstant.BIZ_TEMPLATE_CODE) });
  }
