...
export class NapPagingComponent implements OnInit {
    ...
    this.http.post(URLConstant.GetRefOfficeByOfficeCode, obj).subscribe(
      (response) => {
        if (response["IsAllowAppCreated"] == true) {
          AdInsHelper.RedirectUrl(this.router, ["Nap/ConsumerFinance/Add"], {});
        } else {
  ...
  GetCallBack(ev: any) {
    ...
    if (ev.Key == "Edit") {
      AdInsHelper.RedirectUrl(this.router, ["Nap/ConsumerFinance/Add/Detail"], { "AppId": ev.RowObj.AppId, "WfTaskListId": ev.RowObj.WfTaskListId });
    }
  }
}
