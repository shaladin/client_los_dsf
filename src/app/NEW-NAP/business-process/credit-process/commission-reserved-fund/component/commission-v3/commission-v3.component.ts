...
@Component({
  selector: 'app-commission-v3',
  templateUrl: './commission-v3.component.html'
})
export class CommissionV3Component implements OnInit {
  ...
  async GetContentData() {
    ...
    await this.http.post<NapAppReferantorModel>(URLConstant.GetListAppReferantorWithDetailByAppId, obj).toPromise().then(
      (response) => {
        this.GetDDLContent(response[CommonConstant.ReturnObj], CommonConstant.ContentReferantor);
      });
  ...
  GetDDLContent(ReturnObject, content: string) {
    if (content == CommonConstant.ContentReferantor) {
      for (let i = 0; i < ReturnObject.length; i++) {
        if (ReturnObject[i].AppId == null) return;
        let KVPObj;
        KVPObj = {
  ...
  async GetRuleDataForForm() {
    ...
    await this.http.post(URLConstant.GetAppCommissionRuleV2, obj).toPromise().then(
      (response) => {
        ...
        if (this.BizTemplateCode == CommonConstant.CFRFN4W || this.BizTemplateCode == CommonConstant.CFNA) {
          ResponseObj[0][0][CommonConstant.ReturnObj].RuleDataObjects.ResultSupplier = null;
          ResponseObj[0][0][CommonConstant.ReturnObj].RuleDataObjects.ResultSupplierEmp = null;
          this.HideForm1 = true;
          this.HideForm2 = true;
        }
        if (ResponseObj[0][0][CommonConstant.ReturnObj].RuleDataObjects.ResultSupplier != null || ResponseObj[0][0][CommonConstant.ReturnObj].RuleDataObjects.ResultSupplierEmp) { // For CFNA
          for (let i = 0; i < ResponseObj[0]["length"]; i++) {
            let temp: RuleCommissionObj = ResponseObj[0][i][CommonConstant.ReturnObj].RuleDataObjects;
        ...
        for(let j = 0; j < ResponseObj["length"]; j++){
          if (ResponseObj[j][0][CommonConstant.ReturnObj].RuleDataObjects.ResultReferantor != null){
            for(let k = 0; k < ResponseObj[j]["length"]; k++) {
              let temp: RuleCommissionObj = ResponseObj[j][k][CommonConstant.ReturnObj].RuleDataObjects;
              console.log(this.ContentObjReferantor[j].Key);

              this.BindRuleData(temp.ResultReferantor, CommonConstant.ContentReferantor, this.ContentObjReferantor[j].Key);
            }
