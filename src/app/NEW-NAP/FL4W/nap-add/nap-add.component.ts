...
export class NapAddComponent implements OnInit {
  ...
  MakeLookUpObj() {
    ...
    critObj.value = CommonConstant.FL4W;
    ...
    addCritBizTempalte.value = CommonConstant.FL4W;
  ...
  SaveForm() {
    ...
    this.http.post<GenericObj>(AddNapUrl, requestAddNapObj).subscribe(
      (response) => {
        ...
        AdInsHelper.RedirectUrl(this.router, [NavigationConstant.NAP_FL4W_ADD_DETAIL], { "AppId": response.Id });
      });
  }
  ...
  getLookupAppResponseName(ev: any) {
    let url = URLConstant.GetListProdOfferingDByProdOfferingCode;
