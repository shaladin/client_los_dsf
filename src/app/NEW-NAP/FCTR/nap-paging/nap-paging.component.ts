...
export class NapPagingComponent implements OnInit {
    ...
    this.http.post(URLConstant.GetRefOfficeByOfficeCode, obj).subscribe(
      (response) => {
        if (response["IsAllowAppCreated"] == true) {
          AdInsHelper.RedirectUrl(this.router,["Nap/Factoring/Add"], {});          
        } else {
  ...
  GetCallBack(ev: any) {
    ...
    if (ev.Key == "Edit") {
      AdInsHelper.RedirectUrl(this.router,["Nap/Factoring/Add/Detail"], { "AppId": ev.RowObj.AppId, "WfTaskListId": ev.RowObj.WfTaskListId });
    }
  }
}
