...
export class GuarantorPersonalComponent implements OnInit {
  ...
  PersonalForm = this.fb.group({
    ...
    CustModelCode: ['', Validators.maxLength(50)],
    ...
    MotherMaidenName: ['', [Validators.maxLength(50)]],
  });

  async ngOnInit(): Promise<void> {
    this.inputAddressObjForPersonal = new InputAddressObj();
    this.inputAddressObjForPersonal.title = "Address";
    this.inputAddressObjForPersonal.showAllPhn = true;
    this.inputAddressObjForPersonal.showFax = true;
    this.inputAddressObjForPersonal.showOwnership = true;
    ...
    this.http.post(URLConstant.GetListKeyValueByMrCustTypeCode, custModelReqObj).subscribe(
      (response) => {
        this.CustModelCode = response[CommonConstant.ReturnObj];
        if (this.mode != "edit") {
          this.PersonalForm.patchValue({
            CustModelCode: this.CustModelCode[0].Key
          });
        }
      }
    );
