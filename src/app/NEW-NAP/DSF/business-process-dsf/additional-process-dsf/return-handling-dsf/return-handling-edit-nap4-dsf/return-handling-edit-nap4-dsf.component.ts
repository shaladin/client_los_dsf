...
@Component({
  selector: 'app-return-handling-edit-nap4-dsf',
  templateUrl: './return-handling-edit-nap4-dsf.component.html'
})
export class ReturnHandlingEditNap4DsfComponent implements OnInit, OnDestroy {
    ...
    await this.http.post(URLConstant.GetGeneralSettingByCode, generalSettingObj).toPromise().then(
      async (response) => {
        ...
        { 
          ...
          this.inputPagingObj.title = "Return Handling - Edit Customer Data Completion"
        ...
        critRtnHandlingTask.value = "RTN_EDIT_NAP4";
  ...
  async GetCallback(ev) {
    if (ev.Key == "Edit") {
      ...
      if (this.BizTemplateCode == CommonConstant.CF4W) {
        // Modify by Self Custom
        AdInsHelper.RedirectUrl(this.router, [NavigationConstantDsf.NAP_CUST_COMPL_DETAIL_DSF], { "AppId": ev.RowObj.AppId, "WfTaskListId": wfTaskListIdTemp, "ReturnHandlingHId": ev.RowObj.ReturnHandlingHId });
        // End Modify by Self Custom
      }
