...
@Component({
  ...
  templateUrl: './credit-approval-cfna-paging-dsf.component.html',
  styleUrls: ['./credit-approval-cfna-paging-dsf.component.css']
})
export class CreditApprovalCfnaPagingDsfComponent implements OnInit {
  ...
  GetCallBack(ev: any) {
    ...
    if (ev.Key == "ViewProdOffering") {
      ...
      if (String.Format("{0:L}", ev.RowObj.CurrentUser) != String.Format("{0:L}", this.userContext.UserName)) {
        ...
        AdInsHelper.RedirectUrl(this.router,[NavigationConstantDsf.NAP_CRD_PRCS_CRD_APPRV_CFNA_DETAIL],{ "AppId": ev.RowObj.AppId, "TaskId" : ev.RowObj.TaskId, "InstanceId": ev.RowObj.InstanceId, "MrCustTypeCode": ev.RowObj.MrCustTypeCode, "ApvReqId": ev.RowObj.ApvReqId  });
      }
    }
